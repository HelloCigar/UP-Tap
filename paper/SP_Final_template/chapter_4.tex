...
Figure~ presents the process flow of the UPTap Attendance System, illustrating how the system handles student time-in and time-out operations within a teacher-controlled attendance session. The diagram demonstrates the coordinated interaction between teachers, students, and the system throughout the attendance lifecycle.

\subsubsection*{Teacher Actions}

The process begins with the teacher logging into the system and undergoing authentication. Once authenticated, the teacher gains access to the dashboard and initiates an attendance session. This session serves as the valid window during which students are permitted to record their attendance.

While the session is active, the teacher may optionally monitor attendance progress. Upon concluding the class, the teacher ends the session, which triggers the system to finalize and lock the attendance records. These finalized logs are then available for viewing or exporting as reports.

\subsubsection*{Student Actions and System Behavior}

While an attendance session is active, students are prompted to perform either a time-in or time-out action. Before processing, the system verifies the existence of an active session. If confirmed, the student selects their desired action.

Both time-in and time-out procedures follow the same verification mechanism—using facial recognition or RFID scanning. Once verified, the system records the attendance action along with a timestamp. If a student attempts to interact with the system when no session is active, they are notified accordingly.
