...
const selectedSubject = ref<SubjectOut>()
const selectedSection = ref<string>()
const recordType = ref('time_in')
const newSubject = ref<Subjects>()
const deleted = ref(false)

const { data: subjects } = await useFetch<SubjectOut[]>(() => '/api/teachers/subjects', 
  { 
    method: 'GET',
...
const openEditModal = (subject: SubjectOut, section?: string) => {
  ...
  if (selectedSubject.value) {
    await $fetch('/api/subjects', {
      ...
      body: {
        ...
        subject_name: selectedSubject.value.subject_name,
        section: selectedSection.value,
        schedule: selectedSubject.value.schedule,
        start_time: selectedSubject.value.start_time,
        end_time: selectedSubject.value.end_time
      },
      query: {
        subject_id: selectedSubject.value.subject_id
      }
    ...
    toast.add({
      ...
      description: error.data.data.detail || "Time schedule conflict, please try again",
      color: 'red',
    })
  })
  } else {
...
async function deleteSubject (subject: SubjectOut) {
  modal.open(
    SubjectDeleteModal, {
  ...
  <div class="min-h-screen">
    ...
    <div class="p-8">
      ...
      <div class="grid gap-8 md:grid-cols-2 items-start">
        ...
        <UCard class="flex flex-col h-96">
          ...
          <div class="overflow-scroll h-64 flex-1 space-y-4">
            ...
            <template v-for="subject in subjects" :key="subject.subject_id">
              <!-- Only show subject if current period exists -->
              <template v-if="subject.period.length > 0">
                <template v-for="section in subject.section" :key="section.name">
                  <div class="flex items-center justify-between p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <div>
                      ...
                      <p class="font-thin text-gray-900 dark:text-white">
                    ...
                    <div class="flex gap-2">
                      <button
                        @click="openEditModal(subject, section.name)"
                      ...
                      <CSVImporter :subject_id="subject.subject_id" :subject_name="subject.subject_name" />
                    </div>
                  </div>
                </template>
              </template>
      ...
      <UCard class="mt-8">
        ...
        <SubjectTimeTable v-if="subjects && subjects.length > 0" :subjects="subjects"/>
      </UCard>
    </div>
